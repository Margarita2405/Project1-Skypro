...
def test_success_conversion(mock_get: Mock) -> None:
    ...
    mock_get.assert_called_once_with(
        URL, headers={"apikey": "test-api-key"}, params={"from": "USD", "to": "RUB", "amount": 100}
    )
...
def test_api_error(mock_get: Mock) -> None:
    ...
    mock_response = Mock()
    mock_response.json.return_value = {"success": False, "error": {"info": "Invalid currency"}}
    mock_get.return_value = mock_response
    ...
    assert mock_get.call_count == 1
    mock_get.assert_called_once_with(
        URL, headers={"apikey": "test-api-key"}, params={"from": "INVALID", "to": "RUB", "amount": 100}
    )
...
def test_default_currency(mock_get: Mock) -> None:
    ...
    mock_get.assert_called_once_with(
        URL, headers={"apikey": "test-api-key"}, params={"from": "EUR", "to": "RUB", "amount": 50}
    )
...
def test_explicit_currencies(mock_get: Mock) -> None:
    ...
    mock_get.assert_called_once_with(
        URL, headers={"apikey": "test-api-key"}, params={"from": "USD", "to": "EUR", "amount": 1}
    )
...
def test_zero_amount() -> None:
    ...
    transaction = {"operationAmount": {"amount": "0.0", "currency": {"code": "USD"}}}
